        ...
        <div class="justify-content-center">
         <nav id="pagination" aria-label="Page navigation" style="margin-left:350px;">
	            <ul class="pagination justify-content-center">
		            ...
		            <li class="page-item"><a class="page-link" href="adminpage_reserve1.do?page=${ascurpage>1?ascurpage-1:ascurpage }">Previous</a></li>
					</c:if>
					
		             <c:forEach var="i" begin="${asstartpage }" end="${asendpage }">
		             	<c:if test="${i==ascurpage }">
		             	<li class="page-item"><a class="active" class="active" href="adminpage_reserve1.do?page=${i }">${i }</a></li>
		             	</c:if>
		             	<c:if test="${i!=ascurpage }">
		             	<li class="page-item"><a class="page-link" class="active" href="adminpage_reserve1.do?page=${i }">${i }</a></li>
		             	</c:if>
		             </c:forEach> 
		             
	                <c:if test="${ascurpage<astotalpage }">
	                <li class="page-item"><a class="page-link" href="adminpage_reserve1.do?page=${ascurpage<astotalpage?ascurpage+1:ascurpage }">Next</a></li>
	              </c:if>
	            </ul>
	        </nav>
        </div>
