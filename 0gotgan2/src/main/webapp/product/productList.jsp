	 ...
	 <div class="container">
        <div class="row mb-4">
            <h2 class="col-6 tm-text-primary">
             ...
             <a href="../product/productList.do?type=0"><input type="button" value="전체보기" class="btn btn-sm btn-danger"></a>
            <a href="../product/productList.do?type=1"><input type="button" value="특가" class="btn btn-sm btn-danger"></a>
            <a href="../product/productList.do?type=2"><input type="button" value="신상" class="btn btn-sm btn-info"></a>
            <a href="../product/productList.do?type=3"><input type="button" value="베스트" class="btn btn-sm btn-warning"></a>
        </div>
        <!-- for -->
		...
		<div class="text-center">
			<ul class="pagination">
  	 	  <c:if test="${startpage>1 }">
  	 	  	<li><a href="productList.do?type=${type }&page=${startpage-1 }" >&lt;</a></li> 
  	 	  </c:if>
  	 	  <c:forEach var="i" begin="${startpage }" end="${endpage }">
  	 	     <c:if test="${curpage==i }">
  	 	     <li class="active"><a href="productList.do?type=${type }&page=${i}">${i }</a></li>
  	 	     </c:if>
  	 	     <c:if test="${curpage!=i }">
  	 	     <li class=""><a href="productList.do?type=${type }&page=${i}">&nbsp;${i }</a></li>
  	 	     </c:if>
  	 	  </c:forEach>
  	 	   <c:if test="${endpage<totalpage }">
  	 	    <li><a href="productList.do?type=${type }&page=${endpage+1 }">&gt;</a></li>
  	 	  </c:if>
  	 	  	</ul>
  	 	  </div>
        </div>
